...
import { FileText, Plus, Search, Calendar, HelpCircle, CheckCircle2, RefreshCw, X } from "lucide-react";
...
const MOCK_QUESTIONNAIRES: QuestionnaireTemplate[] = [
  ...
  { id: '3', title: 'Sales Executive Pitch', details: 'Standard behavioral assessment template with 5 core questions focusing on closing and resilience.', number_of_questions: 5, created_at: '2024-01-20T00:00:00Z' },
  { id: '4', title: 'Marketing Manager', details: 'Questions regarding campaign management and creative strategy.', number_of_questions: 7, created_at: '2024-02-15T00:00:00Z' },
];
  ...
  return (
    <div className="flex flex-col gap-6 p-8 rounded-[24px] bg-[var(--surface-1)] border border-[var(--border-color-light)] dark:border-white/10 shadow-sm">
      ...
      <Dialog open={isModalOpen} onOpenChange={(open) => !open && setIsModalOpen(false)}>
        <DialogContent className="sm:max-w-[800px] h-[80vh] p-0 overflow-hidden border-[var(--border-color-light)] bg-[var(--background-color)] shadow-[0_24px_48px_rgba(var(--shadow-rgb),0.12)] sm:rounded-[24px] dark:border-white/[0.09] flex flex-col">
            ...
            <div className="p-6 pb-2 shrink-0">
              <div className="relative">
                ...
                <Input 
                  ...
                  className="h-14 w-full pl-12 rounded-2xl bg-[var(--surface-2)] border-[var(--border-color-light)] dark:border-white/5 text-[15px] shadow-sm"
                />
              </div>
              ...
              {searchQuery && (
                <div className="flex items-center justify-between mt-4 px-2">
                  ...
                  <Button variant="link" size="sm" className="h-auto p-0 text-[var(--primary-color)]" onClick={() => setSearchQuery("")}>
                    Clear Search
                  </Button>
                </div>
              )}
              ...
              {filteredQuestionnaires.length === 0 ? (
                ...
                filteredQuestionnaires.map((q) => (
                  <div 
                    ...
                    className={cn(
                      "flex items-start gap-4 p-5 rounded-[16px] border-2 cursor-pointer transition-all",
                      selectedId === q.id 
                        ? "border-[var(--primary-color)] bg-[var(--primary-color)]/5" 
                        : "border-[var(--border-color-light)] bg-[var(--surface-2)] hover:border-[var(--primary-color)]/30 dark:border-white/5"
                    )}
                  >
                    <div className="flex items-center justify-center w-12 h-12 rounded-xl bg-[var(--surface-3)] shrink-0">
                      <FileText className="h-6 w-6 text-muted-foreground" />
                    </div>
                    <div className="flex flex-col flex-1 gap-2 min-w-0">
                      <div className="flex items-start justify-between gap-4">
                        <h4 className="text-[16px] font-bold text-foreground leading-tight truncate">
                          {q.title}
                        </h4>
                        {selectedId === q.id && (
